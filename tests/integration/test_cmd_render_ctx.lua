...
--      resolve_task_at returns kind='render' with bufnr/lnum/task populated.
--      cancel_one mutates the render-buffer line in-place via nvim_buf_set_lines.
--   3. Fire the write-pre pipeline (mirrors User:ObsidianNoteWritePre).
--      F4 Phase 2 detects the hash mismatch and emits a patch.
--   4. Assert source file is patched and buffer is stripped to fence-only.
--
...
T["cancel on render line → buffer mutated, write-pre patches source, buffer stripped"] = function()
  ...
  -- After render: rows 0-2 = fence, row 3 = "- [ ] Buy milk".
  local bufnr = make_buf({ "```tasks", "not done", "```" })
  render.render_buffer(bufnr)

  -- Verify the task was rendered at row 3 (0-indexed).
  ...
  -- Render buffer line 4 (1-indexed) must be the cancelled task.
