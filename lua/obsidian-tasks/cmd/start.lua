...
local function start_one(resolved, date)
  ...
  elseif resolved.kind == "render" then
    require("obsidian-tasks.log").warn("ObsidianTask start: render lines are updated via edit-through on :w")
  end
end
...
function M.run(args, range)
  ...
  if args and args[1] then
    ...
    if resolved.kind == "render" then
      log.warn("ObsidianTask start: render lines are updated via edit-through on :w")
      return
    end
