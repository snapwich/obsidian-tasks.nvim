...
--   }
-- em_map tracks only the per-task extmarks (not fence / virt_lines marks).
...
---
--- Re-entrant: if the buffer already has an active render, it is cleared first
--- (clear + redraw is atomic from the caller's perspective).
---
...
function M.draw(bufnr, fence_range, layout_lines)
  -- Clear any existing render before drawing (idempotency guarantee).
  ...
  for _, ll in ipairs(layout_lines) do
    ...
    if kind == "label" then
      ...
      vim.api.nvim_buf_set_extmark(bufnr, NS, fence_first, 0, {
    ...
    elseif kind == "task" then
      ...
      if #pending_virt > 0 then
        vim.api.nvim_buf_set_extmark(bufnr, NS, task_lnum, 0, {
...
function M.clear(bufnr)
  ...
  -- Remove inserted task lines first (before clearing extmarks, so ranges are
  -- still correct; extmarks don't affect nvim_buf_set_lines indexing).
  if state.inserted_range then
    local first = state.inserted_range[1]
    local last = state.inserted_range[2]
    vim.api.nvim_buf_set_lines(bufnr, first, last + 1, false, {})
  end

  -- Remove all extmarks owned by our namespace.
...
--- Return the full render state record for a buffer.
--- Used by F4 (render/edit.lua) for diff and patch operations.
---
--- @param bufnr integer
--- @return table|nil  state record or nil if buffer has no active render
